...
public class ReceiptMaker {
	...
	public final int MAX_NUM_ITEMS = 10; // set to max
	public final double TAX_RATE;
	private String[] itemNames; // brackets make it an array
	...
	public ReceiptMaker(){
		...
		itemPrices = new double[MAX_NUM_ITEMS];// set as above
		numItemsPurchased = 0;
	}
	public ReceiptMaker(int maxNumItems, double taxRate){
//		if(isValid(maxNumItems)){
//			MAX_NUM_ITEMS = maxNumItems; //final can't be set to new var
//			}
//		else{
//			
	...
	public int getIndexOfMaxPrice(){
		...
		for(int i=1; i<numItemsPurchased; i++){
			if(itemPrices[i] > itemPrices[indexOfMax]){
				indexOfMax = i; // set to I not price
			}
		}
		return indexOfMax;
	}
	...
	public void displayReceiptStats(){
		System.out.println("\n-----------------RECEIPT STATS-----------------");
		System.out.printf("Min Item Name: %12s | Price: $ %04.2f\n", itemNames[getIndexOfMinPrice()], getMinPrice());// , not +
		System.out.printf("Max Item Name: %12s | Price: $ %04.2f\n", itemNames[getIndexOfMaxPrice()], getMaxPrice());//missing the f in print
